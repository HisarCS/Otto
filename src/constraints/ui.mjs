export function setupConstraintsMenu({ renderer, constraintEngine, displayErrors }) {
  ...
  function fillAnchors(select, shapeName) {
    clear(select);
    if (!shapeName) return;
    try { constraintEngine.rebuild(); } catch(_) {}
  ...
  B_btn.addEventListener('click', () => {
    try {
      if (!B_sa.value || !B_sb.value || !B_aa.value || !B_ab.value) return;
      if (B_sa.value === B_sb.value && B_aa.value === B_ab.value) { alert('Pick different anchors.'); return; }
      ...
      if (typeof window.updateCodeFromConstraints === 'function') window.updateCodeFromConstraints();
      setTimeout(() => { try { window.updateCodeFromConstraints && window.updateCodeFromConstraints(); } catch(_){} }, 0);
      if (typeof window.updateConstraintsMenu === 'function') window.updateConstraintsMenu(constraintEngine.getConstraintList());
    } catch (e) { err(e); }
  });
  ...
  C_btn.addEventListener('click', () => {
    try {
      ...
      if (!Number.isFinite(d) || d < 0) { alert('Enter a non-negative distance.'); return; }
      ...
      window.__pendingConstraints.push({
        type: 'distance',
        a: { shape: C_sa.value, anchor: C_aa.value },
        b: { shape: C_sb.value, anchor: C_ab.value },
        dist: d
      });
      if (typeof window.updateCodeFromConstraints === 'function') window.updateCodeFromConstraints();
      setTimeout(() => { try { window.updateCodeFromConstraints && window.updateCodeFromConstraints(); } catch(_){} }, 0);
  ...
  D_row.children[0].addEventListener('click', () => {
    try {
      if (!D_sa.value || !D_sb.value || !D_aa.value || !D_ab.value) return;
      constraintEngine.addHorizontal(
      ...
      window.__pendingConstraints.push({
        type: 'horizontal',
        a: { shape: D_sa.value, anchor: D_aa.value },
        b: { shape: D_sb.value, anchor: D_ab.value }
      });
      if (typeof window.updateCodeFromConstraints === 'function') window.updateCodeFromConstraints();
      setTimeout(() => { try { window.updateCodeFromConstraints && window.updateCodeFromConstraints(); } catch(_){} }, 0);
      if (typeof window.updateConstraintsMenu === 'function') window.updateConstraintsMenu(constraintEngine.getConstraintList());
  ...
  window.updateConstraintsMenu = function (constraintList) {
    list.innerHTML = '';
